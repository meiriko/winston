...
Logger.prototype.log = function (level) {
  ...
  if( (typeof self.levels[level] === 'undefined') && (typeof self.levels['default'] !== 'undefined') ){
  	args.unshift(level);
	level = 'info';
  }
...
function fixProfileLabel(args){
	var label = args.shift();
	var first = '%s ' + args.shift() ;
	args.unshift(label);
	args.unshift(first);
}
...
Logger.prototype.profile = function (id) {
  ...
  if (this.profilers[id]) {
    then = this.profilers[id].time;
	var logLevel = this.profilers[id].level;
	...
	if(typeof args[args.length - 1] === 'function'){
		if(typeof args[args.length - 2] === 'object'){
			...
			if(args.length > 3){
			  // remove the label, we don't need it
			  fixProfileLabel(args);
			}
		} else {
			if(args.length > 2){
			  // remove the label, we don't need it
	...
	var logFunc = this[logLevel] || this['info'];
    return logFunc.apply(null, args);
  }
  else {
    this.profilers[id] = {time: now, level: (args[args.length -1] || 'info')};
  }
